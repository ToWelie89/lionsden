...
const readMarkdownFilesInDir = async dirname => new Promise(resolve => {
    const ids = [];
    fs.readdir(dirname, async function (err, filenames) {
        ...
        for (let i = 0; i < filenames.length; i++) {
            ...
            if (filename.endsWith('.md')) {
                ...
                const text = getTextBetween(content, '<div class="postId" style="display: none;">', '</div>')
...
const getCarouselChidlren = async (mediaId) => {
    try {
        const data = await fetch(`https://graph.instagram.com/${mediaId}/children?fields=id,media_url,media_type&access_token=${process.env.INSTAGRAM_GRAPH_API_TOKEN}`);
        const json = await data.json();
        return json;
    } catch (err) {
        log('ERROR: Could not fetch carousel children of post with id ' + mediaId);
        mailer.sendErrorLogMail(('ERROR: Could not fetch carousel children of post with id ' + mediaId), JSON.stringify(err, null, 4));
...
const pushAndDeploy = async (postname) => {
    try {
        ...
        await shellCommand(`git commit -m "Automated news feed update from Raspberry Pi ${Date.now()}"`);
        await delay(1000);
        ...
        log('ERROR: Error found when trying to commit and push to Git, as well as deploying the site');
        log(JSON.stringify(err, null, 4));
        await mailer.sendErrorLogMail('Något gick fel när scriptet försökte pusha till git och deploya hemsidan', JSON.stringify(err, null, 4));
    }
}
...
const hexoCreatePost = async (postname) => {
    try {
        ...
        log(JSON.stringify(err, null, 4));
        await mailer.sendErrorLogMail(('ERROR: Could not create hexo post with postname: ' + postname), JSON.stringify(err, null, 4));
    }
}
...
const downloadImage = async (imageURL, downloadFilePath) => {
    try {
        ...
        log('ERROR: Could download the file with url: ' + imageURL + ' to: ' + downloadFilePath);
        log(JSON.stringify(err, null, 4));
        await mailer.sendErrorLogMail(('ERROR: Could download the file with url: ' + imageURL + ' to: ' + downloadFilePath), JSON.stringify(err, null, 4));
    }
}

const run = async () => {
    // Get feed
    const data = await fetch(`https://graph.instagram.com/me/media?fields=id,caption,permalink,thumbnail_url,timestamp,media_url,username,media_type&access_token=${process.env.INSTAGRAM_GRAPH_API_TOKEN}`);
    const json = await data.json();
            ...
            for (let i = 0; i < json.data.length; i++) {
                ...
                if (!handledIds.includes(item.id)) {
                    ...
                    Object.keys(INSTAGRAM_HANDLES).forEach(key => {
                        ...
                        key = key.replace('@', '');
                        const key1 = key.charAt(0).toUpperCase() + key.slice(1).toLowerCase();
                    ...
                    if (!fs.existsSync(markdownPath)) { // Hexo-post for this IG-post doesn't already exists
                        ...
                        if (item.media_type === 'IMAGE') {
                            ...
                            for (var j = 0; j < item.carouselChildren.length; j++) {
                                ...
                                if (child.media_type === 'IMAGE') {
                                    await downloadImage(child.media_url, `${postFolderPath}/${j + 1}.jpg`);
                                }
                            }
                        ...
                        tags:
                        ---
                        <div class="postId" style="display: none;">ID: ${item.id}</div>
                        ...
                        ${item.media_url && item.media_type === 'CAROUSEL_ALBUM' ? `
                            <div
                                ...
                                carousel-children="${item.carouselChildren.length}"
                            >
                                ${item.carouselChildren && item.carouselChildren[0] && item.carouselChildren[0].media_type === 'IMAGE' ? `
                                <div class="carouselChild">
                                    {% asset_img 1.jpg %}
                                </div>
                                ` : ''}
                                ${item.carouselChildren && item.carouselChildren[1] && item.carouselChildren[1].media_type === 'IMAGE' ? `
                                <div class="carouselChild">
                                    {% asset_img 2.jpg %}
                                </div>
                                ` : ''}
                                ${item.carouselChildren && item.carouselChildren[2] && item.carouselChildren[2].media_type === 'IMAGE' ? `
                                <div class="carouselChild">
                                    {% asset_img 3.jpg %}
                                </div>
                                ` : ''}
                                ${item.carouselChildren && item.carouselChildren[3] && item.carouselChildren[3].media_type === 'IMAGE' ? `
                                <div class="carouselChild">
                                    {% asset_img 4.jpg %}
                                </div>
                                ` : ''}
                                ${item.carouselChildren && item.carouselChildren[4] && item.carouselChildren[4].media_type === 'IMAGE' ? `
                                <div class="carouselChild">
                                    {% asset_img 5.jpg %}
                                </div>
                                ` : ''}
                                ${item.carouselChildren && item.carouselChildren[5] && item.carouselChildren[5].media_type === 'IMAGE' ? `
                                <div class="carouselChild">
                                    {% asset_img 6.jpg %}
                                </div>
                                ` : ''}
                                ${item.carouselChildren && item.carouselChildren[6] && item.carouselChildren[6].media_type === 'IMAGE' ? `
                                <div class="carouselChild">
                                    {% asset_img 7.jpg %}
                                </div>
                                ` : ''}
                                ${item.carouselChildren && item.carouselChildren[7] && item.carouselChildren[7].media_type === 'IMAGE' ? `
                                <div class="carouselChild">
                                    {% asset_img 8.jpg %}
                                </div>
                                ` : ''}
                                ${item.carouselChildren && item.carouselChildren[8] && item.carouselChildren[8].media_type === 'IMAGE' ? `
                                <div class="carouselChild">
                                    {% asset_img 9.jpg %}
                                </div>
                                ` : ''}
                                ${item.carouselChildren && item.carouselChildren[9] && item.carouselChildren[9].media_type === 'IMAGE' ? `
                                <div class="carouselChild">
                                    {% asset_img 10.jpg %}
                                </div>
                                ` : ''}
                        ...
                        <div class="automaticGeneratedPostDescription">
                            Detta inlägg genereras automatiskt via vårt Instagram-flöde. För att se original-inlägget klicka <a target="_blank" href="${item.permalink}">här</a>
                        </div>
                        <br>
                        `;
                        ...
                        try {
                            ...
                            log(JSON.stringify(err, null, 4));
                            console.error(err);
                            mailer.sendErrorLogMail('Något gick fel när scriptet försökte pusha till git och deploya hemsidan', JSON.stringify(err, null, 4));
                        }
                    } else {
